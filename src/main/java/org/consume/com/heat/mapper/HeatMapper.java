...
public interface HeatMapper {
     ...
     */
    @Insert({
            "insert into heat_table set uuid = #{model.uuid},names = #{model.names},parents=#{model.parents},mj=#{model.mj}"
    })
     ...
     */
    @Update({
            "update heat_table set names = #{model.names},parents=#{model.parents},mj=#{model.mj} where uuid = #{model.uuid}"
    })
    ...
    @Select({
            "select h.uuid,h.`names`, " +
                    " case h.parents when '0' then '总部' else s.`names` end as parents,h.mj,h.wx  " +
                    " from heat_table h left join structure_table s on s.uuid = h.parents " +
                    " ORDER BY s.ordes,CONVERT(h.`names` USING gbk) asc"
    })
    ...
    @Select({
            "select h.uuid,h.`names`, " +
                    " case h.parents when '0' then '总部' else s.`names` end as parents,h.mj,h.wx  " +
                    " from heat_table h left join structure_table s on s.uuid = h.parents WHERE h.`names` LIKE #{query}" +
                    " ORDER BY s.ordes,CONVERT(h.`names` USING gbk) asc"
    })
    Page<HeatModel> findQueryPage(@Param("query") String query);

    @Select({
            "select * from heat_table order by CONVERT(names USING gbk)"
    })
