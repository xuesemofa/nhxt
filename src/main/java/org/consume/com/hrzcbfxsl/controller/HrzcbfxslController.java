package org.consume.com.hrzcbfxsl.controller;
...
@RestController
@RequestMapping("/hrzcbfxsl")
public class HrzcbfxslController {

    private final static Logger logger = LoggerFactory.getLogger(HrzcbfxslController.class);
    ...
    @Autowired
    private HrzcbfxslService hrzcbfxslService;
    @Autowired
    ...
    public ModelAndView init(@PathVariable("pageNow") Integer pageNow) {
        ...
        return new ModelAndView("/hrzslcbfx/index")
                .addObject("page", all);
    }
    ...
    public ModelAndView qureyid(@RequestParam(value = "id",required = false) String id) {
        ...
        return new ModelAndView("/hrzslcbfx/Exhibition")
                .addObject("list", byId);
    }
...
//        获取今天的数据
        List<HrzdlModel> models1 = hrzcbfxslService.getByJzid(id, strDate1, endDate1);
...
//        获取历史数据
        ...
        for (int i = 1; i < 7; i++) {
            ...
            List<HrzdlModel> models2 = hrzcbfxslService.getByJzid(id, strDate2, endDate2);
            ma.put(i, models2);
        }
    ...
    public ModelAndView init1(@RequestParam(value = "pageNow", required = false) Integer pageNow,
        ...
        Page<ExchangeModel> page = hrzcbfxslService.findAllPage(pageNow, pageSize, serach);
        return new ModelAndView("/hrzslcbfx/aspect")
                .addObject("page", page)
    ...
    public ModelAndView choiceTime(@RequestParam("id") String id) {
        return new ModelAndView("/hrzslcbfx/choiceTime").addObject("jzid", id);
    }
    ...
    public ModelAndView confirmTime(@RequestParam("t") String t, @RequestParam("j") String j) throws Exception {
        ...
        for (int i = 0; i < split.length; i++) {
            ...
            long l = p.getTime();
            long ll = p1.getTime();
            List<SltjModel> byId = sltjService.findByTimeAndId2(j, l, ll);
        ...
        return new ModelAndView("/hrzslcbfx/portrait").addObject("list", obj);
    }
    ...
    public ModelAndView init2(@RequestParam(value = "pageNow", required = false) Integer pageNow,
        ...
        Page<ExchangeModel> page = hrzcbfxslService.findAllPage(pageNow, pageSize, serach);
        return new ModelAndView("/hrzslcbfx/aspect2")
                .addObject("page", page)
                .addObject("A", A)
                .addObject("B", B)
                .addObject("AA", AA)
    ...
    public ModelAndView toTime(@Param("s") String s) {
        return new ModelAndView("/hrzslcbfx/choiceTime2").addObject("jzid", s);
    }
    ...
    public ModelAndView confirmTime2(@RequestParam("t") String t, @RequestParam("j") String j) {
        ...
        for (int i = 0; i < split1.length; i++) {
            List<SltjModel> byId = sltjService.findByTimeAndId2(split1[i], l, ll);
            double d = 0;
            double v = 0;
            if (byId != null && byId.size() > 0) {
                if(byId != null && byId.size() > 0)
        ...
        return new ModelAndView("/hrzslcbfx/transverse2").addObject("list", obj);
    }
}
